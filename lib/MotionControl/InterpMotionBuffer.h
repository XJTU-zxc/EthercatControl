...
public:
    ...
    bool Execute()
    {
        ...
        if (interpMotionBlocks[head].Type() == INTERP_TYPE_LINE) {
            ...
            if(g_params_stm == 1 ||
                    /* 两个运动轴都有限位说明两个轴都停止了 */
              ( ((InterplastLimit[g_params_plate_interp_axis[0] * 2]  == 1 ) || (InterplastLimit[g_params_plate_interp_axis[0] * 2 + 1]  == 1 ))
               && ((InterplastLimit[g_params_plate_interp_axis[1] * 2]  == 1 ) || (InterplastLimit[g_params_plate_interp_axis[1] * 2 + 1]  == 1 )) )
            ){
                isOk = true;
            }
            /* 如果是直线插补，需要输出 */
